...
void __attribute__((visibility("default"))) glBindVertexBuffer(GLuint bindingIndex, GLuint buffer, GLintptr offset, GLsizeiptr stride) {
    ...
    // 创建并更新绑定缓冲区
    glvao_t* vao = glstate->vao;  // 当前绑定的 VAO

    if (vao) {
        vao->vertexattrib[bindingIndex].buffer = glbuffer;  // 更新 VAO 中的绑定索引
        vao->vertexattrib[bindingIndex].pointer = (const void*)offset;  // 设置偏移量
        vao->vertexattrib[bindingIndex].stride = stride;  // 设置步幅
        vao->vertexattrib[bindingIndex].enabled = 1;  // 启用绑定
    }
    else {
        // 如果没有绑定 VAO，抛出错误
        errorShim(GL_INVALID_OPERATION);
...
__attribute__((visibility("default"))) void gl4es_glVertexAttribI2iv(GLuint index, const GLint* v) {
...
__attribute__((visibility("default"))) void glVertexAttribP1ui(GLuint index, GLenum type, GLboolean normalized, GLuint value) {
    ...
    // Update the vertex attribute in the correct VAO (vertex array object)
    if (glstate->vao) {
        glstate->vao->vertexattrib[index].type = type;
        glstate->vao->vertexattrib[index].enabled = 1;
        glstate->vao->vertexattrib[index].normalized = normalized;
        glstate->vao->vertexattrib[index].pointer = (const GLvoid*)(uintptr_t)value;  // Pointer to the value
    }
    else {
        errorShim(GL_INVALID_OPERATION);  // VAO not bound
    }
...
__attribute__((visibility("default"))) void glVertexAttribLPointer(GLuint index, GLint size, GLenum type, GLsizei stride, const void* pointer) {
    ...
    attrib->enabled = 1;
    attrib->normalized = 0;  // 64位浮点数不需要归一化
