...
#ifdef USE_LTW_SHADER_CONVERTER
        //Open ltw.so
        int handle = dlopen("libltw.so", RTLD_NOW | RTLD_GLOBAL);
        if (handle) {
            SHUT_LOGD("[INFO] Using libltw.so as GLSL Converter.");
            typedef unsigned long (*ConvertShaderVgpu_t)(unsigned long shader_code, unsigned int shader_type);
            ConvertShaderVgpu_t ConvertShaderVgpu = (ConvertShaderVgpu_t)dlsym(handle, "ConvertShaderVgpu");
            if (ConvertShaderVgpu)
            {
                glshader->converted = (char *)ConvertShaderVgpu(glshader->source, glshader->type);
                SHUT_LOGD("\n[INFO] [Shader] Converted Shader source: ");
                SHUT_LOGD("%s", glshader->converted);
                dlclose(handle);
            }
            else {
                SHUT_LOGD("[ERROR] ltw_ConvertShaderVgpu is NULL");
                SHUT_LOGD("Failed to find ConvertShaderVgpu function: %s\n", dlerror());
                dlclose(handle);
            }
        }
        else {
            SHUT_LOGD("[ERROR] Failed to open libltw.so.");
        }
#else
